...
public class RNYandexMapKitManager extends SimpleViewManager<MapView> implements UserLocationObjectListener {
    ...
    private LocaleUpdateListener localeUpdateListener=new LocaleUpdateListener(){@Override public void onLocaleUpdated(){

    }

    @Override public void onLocaleUpdateError(@NonNull Error error){

    }};

    private MapObjectTapListener mapObjectTapListener=new MapObjectTapListener(){@Override public boolean onMapObjectTap(@NonNull MapObject mapObject,@NonNull Point point){if(mapObject instanceof PlacemarkMapObject||mapObject instanceof PolygonMapObject){WritableMap writableMap=Arguments.createMap();Object userData=mapObject.getUserData();

    if(userData!=null){try{MarkerUserData markerUserData=(MarkerUserData)userData;writableMap.putMap("data",markerUserData.getData());}catch(Exception e){

    }}

    writableMap.putDouble("latitude",point.getLatitude());writableMap.putDouble("longitude",point.getLongitude());

    String eventName=mapObject instanceof PlacemarkMapObject?PROP_ON_MARKER_PRESS:PROP_ON_POLYGON_PRESS;

    sendNativeEvent(eventName,writableMap,mapView.getId(),context);return true;}return false;}};

    private InputListener inputListener=new InputListener(){@Override public void onMapTap(@NonNull com.yandex.mapkit.map.Map map,@NonNull Point point){WritableMap writableMap=Arguments.createMap();writableMap.putString("latitude",Double.toString(point.getLatitude()));writableMap.putString("longitude",Double.toString(point.getLongitude()));

    sendNativeEvent(PROP_ON_MAP_PRESS,writableMap,mapView.getId(),context);if(shouldSearchLocation){if(searchSession!=null){searchSession.cancel();}

    int zoom=(int)mapView.getMap().getCameraPosition().getZoom();searchSession=searchManager.submit(point,zoom,new SearchOptions(),searchListener);}}
    ...
    private CameraListener cameraListener = new CameraListener() {
        ...
        public void onCameraPositionChanged(@NonNull com.yandex.mapkit.map.Map map, CameraPosition cameraPosition,
            ...
            Boolean isGesture = cameraUpdateSource.name().equals("GESTURES");
    ...
    private DrivingSession.DrivingRouteListener drivingRouteListener=new DrivingSession.DrivingRouteListener(){@Override public void onDrivingRoutes(@NonNull List<DrivingRoute>routes){MapObjectCollection mapObjects=mapView.getMap().getMapObjects();clearPolylines(mapView);

    if(!routes.isEmpty()){for(int i=0;i<routes.size();i++){Polyline polylineGeometry=routes.get(i).getGeometry();if(polylineGeometry.getPoints().size()>0){PolylineMapObject polyline=mapObjects.addPolyline(polylineGeometry);polyline.setStrokeColor(Color.argb(153,194,19,19));polyline.setOutlineWidth(0);polylinesList.add(polyline);break;}}}}

    @Override public void onDrivingRoutesError(@NonNull Error error){}};
    ...
    public MapView createViewInstance(ThemedReactContext context) {
        ...
        if (ActivityCompat.checkSelfPermission(this.context,
            ...
            ActivityCompat.requestPermissions(this.context.getCurrentActivity(),
    ...
    private void submitRouteRequest(ArrayList<Point> points) {
        try {
            ...
            requestPoints
                    .add(new RequestPoint(firstPoint, arrivingPoint, drivingArrivalPoint, RequestPointType.WAYPOINT));
            requestPoints
                    .add(new RequestPoint(secondPoint, arrivingPoint, drivingArrivalPoint, RequestPointType.WAYPOINT));
    ...
    public void zoomIn() {
        mapView.getMap()
                .move(new CameraPosition(mapView.getMap().getCameraPosition().getTarget(),
    ...
    public void setPolygons(MapView view, ReadableArray polygons) {
        ...
        if (polygons != null) {
            for (int i = 0; i < polygons.size(); i++) {
                ...
                PolygonMapObject rect = view.getMap().getMapObjects()
                        .addPolygon(new Polygon(new LinearRing(rectPoints), new ArrayList<LinearRing>()));
    ...
    public void navigateToBoundingBox(ReadableMap northEastRegion, ReadableMap southWestRegions,
        ...
        try {
            CameraPosition cameraPosition = map.getMap()
                    .cameraPosition(new BoundingBox(southWestPoint, northEastPoint));
    ...
    private SearchManager.SuggestListener suggestListener=new SearchManager.SuggestListener(){@Override public void onSuggestResponse(@NonNull List<SuggestItem>suggestItems){WritableMap writableMap=Arguments.createMap();WritableArray suggestResult=Arguments.createArray();

    int suggestionsSize=Math.min(5,suggestItems.size());

    for(int i=0;i<suggestionsSize;i++){WritableMap suggestionObject=Arguments.createMap();suggestionObject.putString("value",suggestItems.get(i).getTitle().getText());suggestionObject.putString("searchText",suggestItems.get(i).getSearchText());suggestResult.pushMap(suggestionObject);}

    writableMap.putArray("suggestions",suggestResult);sendNativeEvent(PROP_ON_SUGGESTIONS_FETCH,writableMap,mapView.getId(),context);}

    @Override public void onSuggestError(@NonNull Error error){}};
    ...
    private void sendNativeEvent(final String eventName, final WritableMap event, final int id,
